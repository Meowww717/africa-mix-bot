...
@dp.callback_query(StateFilter(CreateMeeting.choosing_day), F.data.startswith("day_"))
...
@dp.callback_query(StateFilter(CreateMeeting.choosing_time), F.data.startswith("time_"))
...
@dp.message(StateFilter(JoinMeeting.entering_partner_name))
...
@dp.message(StateFilter(AddPartner.entering_partner_name))
...
async def add_guest_start(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "👤 Введи ім'я гостя.\n"
        "Щоб додати пару — напиши через '+', наприклад: <b>Валя + Льоша</b>",
        parse_mode="HTML"
    )
    await callback.answer()


@dp.message(StateFilter(AddGuest.entering_guest_name))
